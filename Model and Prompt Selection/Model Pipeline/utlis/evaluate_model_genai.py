...
class EvaluateModel():
    def __init__(self, dataset=None, model=None, tokenizer=None, model_type = None,few_shot_prompt=None, device=None, subcription_key = None, endpoint = None) -> None:
      self.model = model
      ...
      self.subcription_key = subcription_key
    ...
    def construct_prompt(self, text, **kwargs):
        ...
        prompt = f"""
            ...
            And create a structured output in JSON-like as follows:
            {output_structure}
    ...
    def construct_data_message(self):
        ...
        for _, row in self.dataset.iterrows():
            self.dataset_prompt.append(generate_message(row))

    def model_based_evaluation(self, file_path=None):
        self.construct_data_message()
        for prompt in self.dataset_prompt:
            ...
            result =  generated_text[len(prompt_text):].strip()